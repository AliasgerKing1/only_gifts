...
import {updateCategory, updateCompany} from '../../../../Services/AdminService'
...
let handleClick = async () => {
  ...
  // Create an object to hold the data you want to send to the backend
    ...
    if (title.trim() !== '') {
      formData.append('category', title.trim());
    }
    ...
    let result = await updateCategory(params.id, formData);
    if (result.data.status === 200) {
      navigate('/auth/admin/category');
    }

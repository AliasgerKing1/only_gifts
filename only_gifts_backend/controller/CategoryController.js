...
routes.post('/',upload.single('image'), async (req,res)=> {
    ...
    routes.put('/:id',upload.single('image'), async (req, res) => {
      try {
        ...
        if (req.file) {
          updateFields.image = `https://only-gifts-517r.onrender.com/category/${new_name}`; // Update the 'image' field
        }
    
        // Use the updateFields object to update only the specified fields in the database
        const result = await Category.updateOne({ _id: id }, { $set: updateFields });
